...
// Add the following route to handle booking page
app.get('/booking/:category/:option', async (req, res) => {
    const { category, option } = req.params;
    ...
    try {
      ...
      // Assuming you want to display details for the first event found
  ...
  app.post('/booking/:category/:option', async (req, res) => {
    try {
      const { name, adultCount, childrenCount, payment, paymentmethod, eventId } = req.body;
  
      // Use eventId to fetch the specific event details from the database
